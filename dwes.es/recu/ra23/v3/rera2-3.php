...
require_once($_SERVER["REQUEST_METHOD"] . "/recu/includes/funciones.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errores_fichero === UPLOAD_ERR_OK) {
        ...
        if (count($array_filtrado) < 4 || count($array_filtrado) == 4 && $datos_validados['itv']) exit(4);
        ...
        <table>
            ...
            <tbody>
                ...
                while ($linea = fgetcsv($fichero)) {
                    if (
                        $linea[0] == $datos_validados['tipo'] &&
                        $linea[1] == $datos_validados['marca'] &&
                        $linea[2] == $datos_validados['antiguedad'] &&
                        $linea[3] == ($datos_validados[''] ? 'Si' : 'No')
                    ) {
                ?>
                        <tr>
                            <td><?= $linea[0] ?></td>
        ...
        <fieldset>
            ...
            <div>
                <?php foreach ($tipos_vehiculo as $key => $value) : ?>
                    <input type="radio" name="tipo" id="tipo" value="<?= $key ?>" <?= (isset($datos_validados['tipo']) &&  $datos_validados['tipo'] == $key) ? 'checked' : '' ?>> <?= $value ?>
                <?php endforeach; ?>
            </div>
            ...
            <select name="marca" id="marca">
                <?php foreach ($marcas_vehiculo as $key => $value) : ?>
                    <option value="<?= $key ?>" <?= (isset($datos_validados['marca']) && $datos_validados['marca']) == $key ? 'selected' : '' ?>> <?= $value ?></option>
                <?php endforeach; ?>
            </select>

            <label for="antiguedad">antiguedad</label>
            <input type="text" name="antiguedad" id="antiguedad" <?= isset($datos_validados['antiguedad']) ? "value='{$datos_validados['antiguedad']}'" : '' ?>>

            <label for="itv">itv</label>
            <input type="checkbox" name="itv" id="itv" <?= isset($datos_validados['itv']) ? 'checked' : '' ?>>
